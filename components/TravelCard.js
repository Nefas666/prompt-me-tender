...
export default function TravelCard({
  ...
  const handleShare = async () => {
    if (navigator.share) {
      ...
      toast.info(
        'Sharing is not supported on this browser. Try copying the URL manually.'
      );
    }
  };
  ...
  const SavedPlansDialog = () => (
    <Dialog>
      <DialogTrigger asChild>
        <Button className="flex items-center gap-2 bg-violet-400 dark:bg-green-400">
              ...
              {savedPlans.map((plan) => (
                <AccordionItem key={plan.id} value={`plan-${plan.id}`}>
                  <AccordionTrigger className="capitalize">
                    {plan.parameters.timeOfYear} Trip - {plan.parameters.numberOfDays} days
                  </AccordionTrigger>
                  <AccordionContent>
                    ...
                    <Button
                      ...
                      variant="destructive"
                      size="sm"
                      className="mt-2 rounded-full bg-red-500 opacity-30 text-slate-200 hover:opacity-100"
                    >
                      Delete
                    </Button>
                  </AccordionContent>
    ...
    <>
      <Card className="w-full max-w-3xl mx-auto overflow-scroll max-h-[580px]">
        ...
        <CardFooter className="flex justify-between">
          <Button
            onClick={handleSave}
            className="flex items-center gap-2 bg-violet-400 dark:bg-green-400"
          >
